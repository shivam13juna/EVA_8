...
class ConvNet(nn.Module):
    def __init__(self, norm_layer=None):
        ...
        #     norm_layer = norm_layer(normalized_shape=)
        ...
        self.convblock3 = nn.Sequential(  # 14x14 > 14x14 | RF 6 | jout=2
            nn.Conv2d(16, 8, 1), nn.ReLU(), nn.Dropout2d(0.05)
        )
        ...
        self.convblock8 = nn.Sequential(  # 7x7 > 5x5 | RF 26 | jout=3
            nn.Conv2d(8, 10, 3), nn.ReLU(), nn.Dropout2d(0.05)
        )
        self.convblock9 = nn.Sequential(  # 5x5 > 3x3 | RF 32 | jout=3
            nn.Conv2d(10, 16, 3), nn.ReLU(), nn.Dropout2d(0.05)
        )
        self.convblock10 = nn.Sequential(  # 3x3 > 3x3 | RF 38 | jout=3
            nn.Conv2d(16, 16, 1)
            # nn.ReLU(),
            # norm_layer(normalized_shape=10),
            # nn.Dropout2d(0.1)
        )

        self.view = custom_view()
        # now add a fully connected layer with 10 outputs
